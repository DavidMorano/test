/* testbase128_main SUPPORT */
/* charset=ISO8859-1 */
/* lang=C++20 */

/* test the |base128(3uc)| subroutines */
...
/*******************************************************************************
	...
	Description:
	This program tests the BASE128 object.
...
#define	OUTLEN		BASE128_OUTLEN

#define	BASE128_LINELEN	76
#define	BASE128_BUFLEN	((BASE128_LINELEN / BASE128_OUTLEN) * BASE128_STAGELEN)

#undef	BUFLEN
#define	BUFLEN		(100 * BASE128_BUFLEN)
...
local int outbase128(MI *,cchar *,int) noex ;
...
int maininfo::procgroup(bufos *obp,cchar *sp,int sl) noex {
	...
	if (sl >= stagelen) {
	    ...
	    rs = SR_OK ;
	    if (cint len = base128_e(sp,stagelen,outbuf) ; len > 0) {
...
int maininfo::procpartial(bufos *obp) noex {
	...
	if (obp) {
	    ...
	    cchar	*sp = stagebuf ;
	    char	outbuf[OUTLEN+1] ;
	    if (cint len = base128_e(sp,sl,outbuf) ; len > 0) {
	        rs = obp->add(outbuf,len) ;
	    }
	} /* end if (ok) */
	return rs ;
...
int maininfo::putflush(bufos *bp) noex {
	...
	if (fbuf) {
	    int	 c = 0 ;
	    cint mlen = min(BASE128_LINELEN,flen) ;
	    while ((rs = bp->read(fbuf,mlen)) > 0) {
		if ((rs = bp->adv(rs)) >= 0) {
		    c += rs ;
	            rs = outbase128(this,fbuf,rs) ;
		    olen += rs ;
		}
		if (rs < 0) break ;
	    } /* end while */
...
local int outbase128(MI *pip,cchar *sbuf,int slen) noex {
	...
	char		linebuf[BASE128_LINELEN + 4] ;
	for (int rlen = slen, i = 0 ; (rs >= 0) && (rlen > 0) ; ) {
	    cint	mlen = MIN(BASE128_BUFLEN,rlen) ;
	    if (cint len = base128_e(sbuf + i,mlen,linebuf) ; len > 0) {
	        if (pip->fl.test) {
	            rs = 0 ;
	            if (len > 0) {
	                int	c = 1 ;
