...
/* look up some miscellaneous stuff in various databases */
	...
	rs = ftpsession_open(&weather,hostname,
		"anonymous","morano@ele.uri.edu",NULL) ;

#if	CF_DEBUG
	if (pip->debuglevel > 1)
	    debugprintf("main: ftpsession_open() rs=%d\n",rs) ;
#endif

	rs = ftpsession_fileread(&weather,portspec) ;

#if	CF_DEBUG
	if (pip->debuglevel > 1)
	    debugprintf("main: ftpsession_fileread() rs=%d\n",rs) ;
#endif


	while ((rs = ftpsession_read(&weather,buf,BUFLEN)) > 0) {
...
#if	CF_DEBUG
	if (pip->debuglevel > 1)
	    debugprintf("main: ftpsession_read() rs=%d\n",rs) ;
#endif

	rs = ftpsession_close(&weather) ;

#if	CF_DEBUG
	if (pip->debuglevel > 1)
	    debugprintf("main: ftpsession_close() rs=%d\n",rs) ;
#endif
