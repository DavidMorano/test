...
local int bprintmsg(bfile *,char *,int) noex ;
...
int main(int argc,mainv argv,mainb envv) {
	...
	char	*dialspec = nullptr ;
	char	*hostname = nullptr ;
	char	*portspec = nullptr ;
	char	*svcspec = nullptr ;
	char	*cp ;
...
/* check arguments */
	...
	if (f_log && ((logfname == nullptr) || (logfname[0] == '\0'))) {
...
/* parse some stuff */
	...
	if ((dialspec != nullptr) && (dialspec[0] != '\0')) {

	    if ((cp = strchr(dialspec,':')) != nullptr) {
...
/* look up some miscellaneous stuff in various databases */
	...
	case dialer_udp:
	    ...
	    if ((portspec == nullptr) || (portspec[0] == '\0')) {
	        ...
	        if ((svcspec != nullptr) && (svcspec[0] != '\0'))
	        	portspec = svcspec ;
	...
	case dialer_tcpmux:
	    rs = dialtcpmux(hostname,portspec,AF_INET,svcspec,nullptr,timeout,0) ;
...
local int bprintmsg(bfile *ofp,char *buf,int buflen) noex {
	struct sysmisc_request	m0 ;
	struct sysmisc_loadave	m1 ;
