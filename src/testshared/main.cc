/* main SUPPORT (testshared) */
/* lang=C++20 */

/* test loading a shared-object file using |slopen(3dl)| */
...
#include	<sys/types.h>
#include	<sys/param.h>
#include	<dlfcn.h>		/* for |dlopen(3dl)| */
...
static int sub4() noex {
	...
	void		*handle ;
	cout << "sub=" << "sub4" << eol ;
	handle = dlopen("./zero.so",lm) ;
	cout << "handle=" << handle << " err=" << dlerror() << eol ;
	return rs ;
}
