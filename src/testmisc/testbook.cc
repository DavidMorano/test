...
local int	procword(char *,cchar *,int) ;
...
local int procword(char *tbuf,cchar *cp,int cl) {
	return strwcpylc(tbuf,cp,cl) - tbuf ;
} /* end subroutine (procword) */

local int mkterms(uchar *wterms) {
	int		i ;
