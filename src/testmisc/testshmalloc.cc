...
static int subinfo_start(SUBINFO *,time_t,FILE *,SHMALLOC *,char *,int,int) ;
...
int main(int argc,cchar **argv,cchar **envv)
{
	...
	shsize = (ps * SHSIZEMULT) ;
	fprintf(ofp,"shsize=%u\n",ps) ;
	if ((rs = uc_malloc(shsize,&strp)) >= 0) {
	    SHMALLOC	sh ;
	    if ((rs = shmalloc_init(&sh,strp,shsize)) >= 0) {
	        if ((rs = subinfo_start(sip,dt,ofp,&sh,strp,shsize,ps)) >= 0) {
		...
		SHMALLOC *shp,char *strp,int shsize,int ps)
{
	...
	sip->ps = ps ;
	seed = (dt & UINT_MAX) ;
	if ((rs = randomvar_start(&sip->rng,0,seed)) >= 0) {
	    rs = requests_start(&sip->as) ;
	    if (rs < 0)
	        randomvar_finish(&sip->rng) ;
	}
	return rs ;
}
...
static int subinfo_phase1(SUBINFO *sip)
{
	...
	if ((rs = subinfo_populate(sip)) >= 0) {
	    c = rs ;
	    if ((rs = shmalloc_used(sip->shp)) >= 0) {
	        fprintf(ofp,"used=%d\n",rs) ;
#if	CF_DEBUGS
	        debugprintf("subinfo_phase1: shmalloc_used() rs=%d\n",rs) ;
#endif
	        if ((rs = shmalloc_audit(sip->shp)) >= 0) {
	        ...
	        } /* end if (shmalloc_audit) */
	    } /* end if shmalloc_used) */
...
static int subinfo_populate(SUBINFO *sip)
{
	...
	while (rs >= 0) {
	    if ((rs = subinfo_getrand(sip)) >= 0) {
	        ...
	        if (asize == 0) asize = 1 ;
#if	CF_DEBUGS
	        debugprintf("subinfo_populate: shmalloc_alloc() asz=%u\n",
	            asize) ;
#endif
...
#endif
	        if ((rs = shmalloc_already(shp,ai.ro)) == 0) {
	            if ((rs = shmalloc_free(shp,ai.ro)) >= 0) {
	                if ((rs = shmalloc_avail(shp)) >= 0) {
	                    f = TRUE ;
...
#endif
	            if ((rs = shmalloc_already(sip->shp,ai.ro)) == 0) {
	                if ((rs = shmalloc_free(sip->shp,ai.ro)) >= 0) {
	                    if ((rs = shmalloc_avail(sip->shp)) >= 0) {
