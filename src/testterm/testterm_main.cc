/* testterm_main SUPPORT (termtest) */
/* charset=ISO8859-1 */
/* lang=C++20 */

/* program to display characters */
/* version %I% last-modified %G% */

#define	CF_DEBUG	0
#define	CF_WHO		0
#define	CF_SIGACTION	1
#define	CF_ALARM	1
#define	CF_TESTRAW	0
#define	CF_SLEEP	0

/* revision history:

	= 1986-01-10, David A­D­ Morano
	This was originally written.

*/

/* Copyright © 1986 David A­D­ Morano.  All rights reserved. */
/* Use is subject to license terms. */

/************************************************************************

  	Description:
	This program is used to perform some rudimetary testing
	of the terminal.

***************************************************************************/

#include	<envstandards.h>	/* ordered first to configure */
#include	<sys/types.h>		/* POSIX® */
#include	<sys/unistd.h>		/* POSIX® */
#include	<termios.h>		/* POSIX® */
#include	<fcntl.h>		/* POSIX® */
#include	<cerrno>		/* CSTD */
#include	<csignal>		/* CSTD */
#include	<cstddef>		/* CSTD */
#include	<cstdlib>		/* CSTD */
#include	<clanguage.h>		/* LIBU */
#include	<usysbase.h>		/* LIBU */
#include	<field.h>		/* LIBUC */
#include	<localmisc.h>		/* LIBU */


/* local defines */

#ifndef	LINEBUFLEN
#define	LINEBUFLEN	2048
#endif

#define	TTY_READCHARS	10

#define	TTY_MINCHARS	0		/* minimum characters */
#define	TTY_MINTIME	10		/* x100 milliseconds */


/* external subroutines */


/* external variables */


/* forward references */

void		int_alarm() ;
void		int_signal() ;


/* gloabal variables */

int		f_alarm ;
int		f_signal ;


/* exported variables */


/* exported subroutines */

int main(int argc,con mainv argv,con mainv envv) {
	sigset_t	signalmask ;
	termios		ots, nts ;
	sigaction	sigs ;
	bfile	input, *ifp = &input ;
	bfile	output, *ofp = &output ;
	bfile	error, *efp = &error ;
	int	rs ;
	int	len ;
	int	i, j ;
	int	tfd = -1 ;
	int	f_reveal = FALSE ;

	uchar	c ;
	uchar	buf[LINEBUFLEN], *bp = buf ;


	if (bopen(efp,BFILE_STDERR,"dwca",0664) >= 0)
	    bcontrol(efp,BC_NOBUF,0) ;

	if (bopen(ofp,BFILE_STDOUT,"dwct",0666) < 0)
	    return BAD ;

#ifdef	COMMENT
	bcontrol(ofp,BC_NOBUF) ;
#endif

	if (argc > 1)
	    f_reveal = true ;


/* set up terminal for raw access */

	ioctl(tfd,TCGETS,&ots) ;

	nts = ots ;

	nts.c_iflag &= 
	    (~ (INLCR | ICRNL | IUCLC | IXANY | ISTRIP | INPCK | PARMRK)) ;
	nts.c_iflag |= IXON ;

	nts.c_cflag &= (~ (CSIZE)) ;
	nts.c_cflag |= CS8 ;

#if	CF_TESTRAW
	nts.c_lflag = 0 ;
#else
	nts.c_lflag &= (~ (ICANON | ECHO | ECHOE | ECHOK | ECHONL)) ;
#endif

#if	CF_WHO
	nts.c_lflag &= (~ (ISIG)) ;
#endif

	nts.c_oflag &= (~ (OCRNL | ONOCR | ONLRET)) ;

	nts.c_cc[VMIN] = TTY_MINCHARS ;
	nts.c_cc[VTIME] = TTY_MINTIME ;

/* set it! */

	ioctl(tfd,TCSETSW,&nts) ;


/* set up the handler for alarms */

#if	CF_ALARM
#if	CF_SIGACTION
	(void) sigemptyset(&signalmask) ;

	sigs.sa_handler = int_alarm ;
	sigs.sa_mask = signalmask ;
	sigs.sa_flags = 0 ;
	sigaction(SIGALRM,&sigs,NULL) ;
#else
	signal(SIGALRM,int_alarm) ;
#endif

	f_alarm = FALSE ;
	alarm(4) ;
#endif /* CF_ALARM */

/* set up the handler for interrupts */

	f_signal = FALSE ;
	(void) sigemptyset(&signalmask) ;

	sigs.sa_handler = int_signal ;
	sigs.sa_sigaction = int_signal ;
	sigs.sa_mask = signalmask ;
	sigs.sa_flags = SA_RESTART ;

	sigaction(SIGINT,&sigs,NULL) ;


/* go through the loops */

	j = 0 ;
	while (true) {

	    bflush(ofp) ;

#if	CF_SLEEP
#ifdef	COMMENT
	    system("sleep 3") ;
#else
	    sleep(3) ;
#endif
#endif /* CF_SLEEP */

	    memset(buf,0,TTY_READCHARS) ;

	    if ((len = read(tfd,buf,TTY_READCHARS)) < 0)
		len = (- errno) ;

#if	CF_ALARM
	    if (f_alarm) {

#if	(! CF_SIGACTION)
	        signal(SIGALRM,int_alarm) ;
#endif

	        f_alarm = FALSE ;
	        if (f_reveal) 
			bprintf(ofp," Alarm len=(%d)\n",len) ;

	        alarm(4) ;

	    } /* end if (alarm) */
#endif /* CF_ALARM */

	    if (f_signal) {

	        f_signal = FALSE ;
	        bprintf(ofp," Signal len=(%d)\n",len) ;

	    }

	    if (len < 0) {

	        if (f_reveal) {

	            j = 0 ;
	            bprintf(ofp," len=(%d)\n",len) ;

	        }

	        continue ;

	    } else if (len == 0) {

	        if (f_reveal)
			bprintf(ofp," TO") ;

	        if (j > 0) {

	            j = 0 ;
	            bprintf(ofp,"\n") ;

	        }

	        continue ;

	    } /* end if */

	    for (i = 0 ; i < len ; i += 1) {

	        bprintf(ofp," %02X",buf[i]) ;

	        if ((buf[i] & 0x7F) == '\r') 
			goto done ;

	        if ((buf[i] & 0x7F) == '\n') 
			goto done ;

	        j += 1 ;
	        if (j > 20) {

	            j = 0 ;
	            bprintf(ofp,"\n") ;

	        }

	    } /* end for */

	} /* end while */

done:
	bprintf(ofp,"\n") ;

	ioctl(tfd,TCSETSW,&ots) ;

	bclose(ofp) ;

	bclose(efp) ;

	return 0 ;

/* bad */
badret:
	bclose(efp) ;

	return 1 ;
} /* end subroutine (main) */

void int_alarm(int sig) noex {
	f_alarm = true ;
} /* end subroutine */

void int_signal(int sig) noex {
	f_signal = true ;
} /* end subroutine */


