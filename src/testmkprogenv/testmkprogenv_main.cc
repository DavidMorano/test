/* testmktestprogenv_main SUPPORT */
/* charset=ISO8859-1 */
/* lang=C++20 */
...
int main(int argc,con mainv argv,con mainv envv) {
	...
	int		rs = SR_OK ;
	int		rs1 ;
	[[maybe_unused]] cchar		*fn = "testmkprogenv_envfile.txt" ;
	...
	if (rs >= 0) {
	    if (mkprogenv m ; (rs = mkprogenv_start(&m,envv)) >= 0) {
	        DEBUGPRINTF("mkprogenv-start() rs=%d\n",rs) ;
	        if (mainv ev ; (rs = mkprogenv_getvec(&m,&ev)) >= 0) {
	            DEBUGPRINTF("mkprogenv-getvec() rs=%d\n",rs) ;
		    for (int i = 0 ; ev[i] ; i += 1) {
		        printf("i=%02u %s\n",i,ev[i]) ;
		    } /* end for */
	        } /* end if (mkprogenv_getvec) */
	        DEBUGPRINTF("mkprogenv-getvec-out rs=%d\n",rs) ;
	        rs1 = mkprogenv_finish(&m) ;
	        if (rs >= 0) rs = rs1 ;
	    } /* end if (mkprogenv) */
	    DEBUGPRINTF("mkprogenv-out rs=%d\n",rs) ;
	} /* end if (ok) */
