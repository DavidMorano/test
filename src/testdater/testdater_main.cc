...
#define	CF_INITnullptr	0		/* initialize |dater()| w/ nullptr */
...
local int	process(PI *,dater *,cchar *) noex ;
local int	procin(PI *,dater *,bfile *) noex ;
local int	procline(PI *,dater *,bfile *,cchar *,int) noex ;

#if	CF_DUMPZINFO
local int	dumpzinfo(dater *,cchar *) noex ;
#endif /* CF_DUMPZINFO */
...
/* other */
	...
	if (TIMEB now ; (rs = uc_ftime(&now)) >= 0) {
	    rs1 = SR_INVALID ;
	    if (tmtime tmt ; (rs = tmtime_timelocal(&tmt,now.time)) >= 0) {
	...
	if (dater d ; (rs = dater_start(&d,np,np,0)) >= 0) {
...
local int process(PI *pip,dater *dp,cchar *ofn) noex {
...
local int dumpzinfo(dater *dp,cchar *fname) noex {
	...
	if ((rs = bopen(zfp,fname,"wct",0666)) >= 0) {
	    for (int i = 0 ; dater_zinfo(dp,&zi,i) >= 0 ; i += 1) {
