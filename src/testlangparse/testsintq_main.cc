/* testsintq_main SUPPORT (testsintq) */
/* charset=ISO8859-1 */
/* lang=C++20 */
...
local int procfile(sintq *,cchar *) noex ;
local int procln(sintq *,cchar *,int) noex ;
local int procout(sintq *,int) noex ;
...
int main(int argc,mainv argv,mainv) {
	...
	if (argc > 1) {
	    DEBPRINTF("argc=%d\n",argc) ;
	    if (sintq pa ; (rs = pa.start) >= 0) ylikely {
...
local int procfile(sintq *lpp,cchar *fn) noex {
    	cnothrow	nt{} ;
    	cnullptr	np{} ;
...
local int procln(sintq *lpp,cchar *lp,int ll) noex {
    	int		rs ;
	int		c = 0 ;
...
local int procout(sintq *lpp,int slen) noex {
    	cnullptr	np{} ;
    	cnothrow	nt{} ;
	cint		chs = CH_NL ;
    	int		rs = SR_OK ;
