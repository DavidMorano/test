/* bufslide_main SUPPORT */
/* charset=ISO8859-1 */
/* lang=C++20 */

/* test the BUFSLIDE object */
...
/*******************************************************************************
	...
	Description:
	This program tests the BUFSLIDE object.
...
struct maininfo {
	...
	int enc_textload(bufslide *,cchar *) noex ;
	int enc_textflush(bufslide *) noex ;
	int enc_binary(cchar *) noex ;
	int procln(bufslide *,cchar *,int) noex ;
	int procdata(bufslide *,cchar *,int) noex ;
	int procstage(bufslide *,cchar *,int) noex ;
	int procgroup(bufslide *,cchar *,int) noex ;
	int procout(bufslide *) noex ;
	int procouter(bufslide *) noex ;
	int putout(bufslide *bp,int ch) noex ;
	int putflush(bufslide *bp) noex ;
	void dtor() noex ;
...
cbool		f_text	= true ;	/* <- always for this test */
...
int maininfo::enc_textload(bufslide *obp,cchar *fn) noex {
    	int		rs = SR_OK ;
	int		rs1 ;
	int		olen = 0 ;
	DPRINTF("ent\n") ;
...
int maininfo::enc_textflush(bufslide *obp) noex {
    	int		rs = SR_NOMEM ;
	int		olen = 0 ;
	DPRINTF("ent\n") ;
	if (obp) {
...
int maininfo::procln(bufslide *obp,cchar *lp,int ll) noex {
    	int		rs = SR_OK ;
...
int maininfo::procdata(bufslide *obp,cchar *lp,int ll) noex {
...
int maininfo::procgroup(bufslide *obp,cchar *sp,int sl) noex {
    	int		rs = SR_OK ;
	int		c = 0 ;
	if (sl > 0 ) {
	        rs = obp->add(sp,sl) ;
...
int maininfo::procout(bufslide *obp) noex {
...
int maininfo::procouter(bufslide *obp) noex {
    	cint		linelen = LINELEN ;
    	int		rs ;
	int		olen = 0 ;
	DPRINTF("ent\n") ;
