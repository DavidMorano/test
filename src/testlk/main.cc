...
/*******************************************************************************
	...
	programs.  This subroutine was originally part of the
...
extern int	procfileenv(cchar *,char *,VECSTR *) ;
...
/* the remove-file multiplier */
	...
	if (to_remove < 0) {
		...
		if (mul_remove > 1)
			...
			to_remove = to_lkwait * TESTLK_MULREMOVE ;
#else
		to_remove = TESTLK_DEFREMOVE ;
#endif
...
/* process any files in the argument filename list file */
	...
	if (DEBUG_DEBUG)
	        ...
	        while ((len = breadln(afp,buf,BUFLEN)) > 0) {
	...
	if ((pan == 0) && ((cp = getenv(TESTLK_NAMEVAR)) != NULL)) {
